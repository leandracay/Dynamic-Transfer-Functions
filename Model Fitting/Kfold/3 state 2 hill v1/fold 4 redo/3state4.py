...
if rank == 0:
    ...
    rawdata = trainendpts[3]
    newdata = list(rawdata.flatten())
    all_parameters = (np.delete(np.genfromtxt('params34.csv', delimiter=','),0,0))
        ...
        for j in range(len(parameters[:,0])):
           # sys.stdout.write("I am process %d of %d at %s.\n on %s.\n "  % (rank, size,runtime, name))
            ...
            def model(z,t):
                ...
                dPbdt = k1*I(t)*Pu + d2*Pa - d1*Pb - k2*I(t)**n/(Kd**n+I(t)**n)*Pb
...
if rank == 0:
    ...
    np.savetxt('3model_out_fold4.csv',model1,delimiter=',')
   # print('end = ', model1)
   # print(ydata)
    with open('3model_R2_fold4.csv','w') as f:
