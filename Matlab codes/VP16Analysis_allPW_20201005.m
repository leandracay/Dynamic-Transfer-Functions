...
Frequency1=1./10.^log_Freq;
unique_Freq=sort(unique(Frequency1));
...
for i=1:5
    ...
    for k=1:length(unique_Freq)
       temp=range(foldChangeNew(PW==PW_unique(i) & Frequency1==unique_Freq(k)));
       if isempty(temp)==1
           range_AM(i,k)=NaN;
       else
           range_AM(i,k)=temp;
...
for i=1:4
    for j=1:length(unique_Freq)
        temp=range(foldChangeNew(amplitude==amp_unique(i) & Frequency1==unique_Freq(j)));
        if isempty(temp)==1
            range_PW(i,j)=NaN;
        else
            range_PW(i,j)=temp;
...
[XX,YY]=meshgrid(9:.1:11,-3.5:1:0);
ZZ=b2(1)+b2(2).*XX+b2(3).*log10(5)+b2(4).*YY;
figure
scatter3(log_amp(log_PW==log10(5)),log_Freq(log_PW==log10(5)),log_FC(log_PW==log10(5)))
hold on
surf(XX,YY,ZZ,'facecolor','r')
xlabel('Amplitude'),ylabel('Frequency'),zlabel('FC')
hold off
