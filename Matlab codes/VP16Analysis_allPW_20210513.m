...
PW=allData(:,4);
amplitude=allData(:,5);%units of au
% amplitude=10^(1.0991.*log10(amplitude)-8.905);%nW
% areaOfProbe=(600/2)^2*pi;%um^2
% areaOfProbe=areaOfProbe.*(1/10000)^2;%cm^2
% amplitude=amplitude./areaOfProbe;%nW/cm^2
AUC=allData(:,6);%au*hr
% AUC=PW./inverseFrequency.*amplitude.*14;
...
%% Comparing Same AUC conditions
size_marker(amplitude==6e9)=70;
size_marker(amplitude==1.2e10)=160;
size_marker(amplitude==4e10)=250;
size_marker(amplitude==6e10)=380;
figure
hold on
%t=table(PW(AUC=4.2e10),
startAUC=5e11;
endAUC=6e11;
for i=1:5
scatter(Frequency(PW==unique_PW(i) & AUC>=startAUC & AUC<=endAUC),foldChangeNew(PW==unique_PW(i) & AUC>=startAUC & AUC<=endAUC),size_marker(PW==unique_PW(i) & AUC>=startAUC & AUC<=endAUC),color(i,:),shape_marker(i),'filled')
end
...
title(strcat('AUC=', string(startAUC),'-',string(endAUC)))
%saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_FCvsFreq.png')
hold off
%%
...
size_marker(amplitude==6e9)=150;
size_marker(amplitude==1.2e10)=150;
size_marker(amplitude==4e10)=150;
size_marker(amplitude==6e10)=150;
...
xlim([5e9,1e11])
set(gca,'linewidth',2.5,'ticklength',[0.015 0.015])
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_FCvsIntensity.png')
hold off
hold off
...
hold off
set(gca,'linewidth',2.5,'ticklength',[0.015 0.015])
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_CVvsIntensity.png')
hold off
...
hold off
set(gca,'linewidth',2.5,'ticklength',[0.015 0.015])
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_FCvsAUC.png')
hold off
...
set(gca,'fontsize',13)
set(gca,'linewidth',2.5,'ticklength',[0.015 0.015])
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_CVvsAUC.png')
hold off
...
set(gca,'fontsize',13)
set(gca,'linewidth',2.5,'ticklength',[0.015 0.015])
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_CVvsFreq.png')
hold off
%% 
...
saveas(gcf,'/Volumes/GoogleDrive/My Drive/Shared folder/YeastLightExperiments/VP16 focused matrix/all_CVvsFC.png')
hold off
...
log_Freq=log10(1./inverseFrequency);
log_FC=log10(foldChangeNew);
log_PW=log10(PW);
log_amp=log10(amplitude);
...
figure
[c,m,h,nms]=multcompare(stats_range);
[nms num2cell(m)]
...
X3=[ones(length(zPW),1),zAmplitude',zPW',zFrequency',zFrequency'.*zAmplitude',zFrequency'.*zPW',zAmplitude'.*zPW',zFrequency'.*zAmplitude'.*zPW'];
X2=[ones(length(log_PW),1),log_amp',log_PW',log_Freq'];
zFC=zscore(log_FC');
[b,bint,r,~,stat]=regress(zFC,X);
[b2,bint2,r2,~,stat2]=regress(log_FC',X2)
[b3,bint3,r3,~,stat3]=regress(zFC,X3)
%VIP score
...
%% Imax of all conditions for all four replicates plot (calculated from previous section)
...
plot(stats_MI.number,stats_MI.mean_MI,'.','color',[0.13,.25,.6],'Markersize',30)
plot(n,Imax_all,'.','color',[.5,.5,.5],'Markersize',15)
